...
class MemoryStorage:
    ...
    async def store(self, content: str, tags: List[str] = None, metadata: Dict = None) -> ToolResult:
        ...
        try:
            ...
            if len(self.short_term_memory) > DEFAULT_SHORT_TERM_CAPACITY:
                self.short_term_memory.pop(0)  # Remove oldest memory if capacity exceeded
    ...
    def _search_short_term(self, query: str = None, tags: List[str] = None) -> List[Dict]:
        ...
        for result in results:
            ...
            frequency_score = min(result["access_count"] / 10.0, 1.0)  # Cap at 1.0
            
            # Combined score
            result["score"] = (RECENCY_WEIGHT * recency_score) + (FREQUENCY_WEIGHT * frequency_score)
    ...
    async def _search_long_term(self, query: str = None, tags: List[str] = None, limit: int = 5) -> List[Dict]:
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                params = [RECENCY_WEIGHT, FREQUENCY_WEIGHT, MEMORY_HALF_LIFE_DAYS]
                where_clauses = []
                
                # Add content search if query provided
                if query:
                    where_clauses.append("m.content ILIKE $" + str(len(params) + 1))
                    params.append(f"%{query}%")
                
                # Add tag filtering if tags provided
                if tags:
                    where_clauses.append("mt.tag = ANY($" + str(len(params) + 1) + ")")
                    params.append(tags)
    ...
    async def forget(self, memory_id: int = None, older_than_days: int = None) -> ToolResult:
        ...
        try:
            ...
            if memory_id:
                ...
                self.short_term_memory = [m for m in self.short_term_memory if m.get("id") != memory_id]
                
                # Remove from long-term memory
                async with self.db_pool.acquire() as conn:
                    result = await conn.execute('DELETE FROM memories WHERE id = $1', memory_id)
                    count = int(result.split()[-1])  # Parse count from DELETE X
            ...
            # Handle forgetting by age
                ...
                self.short_term_memory = [m for m in self.short_term_memory if m["created_at"] > cutoff_date]
